...
- Primary skill: `agent-context-architecture`.
...
- Primary skill: `agent-retrieval-architecture`.
- Handoff: `agent-context-architecture` only if durable user memory is being mixed into the answer.
...
- Primary skill: `agent-durability-ops`.
- Handoff: `agent-fallback-eval` only for user-visible degraded response or retry messaging.
...
- Primary skill: `agent-proactive-architecture`.
...
- Primary skill: `agent-fallback-eval`.
- Handoff: `agent-durability-ops` only if crash recovery, checkpointing, or job lifecycle is the main issue.
...
- Primary skill: `agent-frictionless-design`.
- Handoff: `agent-fallback-eval` only if the tool has actually timed out or degraded.
...
- Primary skill: `agent-context-architecture`.
- Handoff: `agent-retrieval-architecture` only for explicit document or conversation search after scope is decided.
...
- Primary skill: `agent-durability-ops`.
- Handoff: `agent-fallback-eval` for degraded response wording, retry/failover policy, and eval seed.
...
- Primary skill: `agent-proactive-architecture` if the core issue is wake timing, autonomy tier, approval, or silence.
- Handoff: `agent-frictionless-design` when the allowed proactive surface still has too much friction.
...
1. Deeper formal experiment design for `agent-frictionless-design`.
2. Cross-device restoration and backup-policy deep dive for `agent-durability-ops`.
