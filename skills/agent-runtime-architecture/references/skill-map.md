...
- If the user says "it forgets" or "wrong context", prefer `agent-context-architecture`.
- If the user says "search", "RAG", "docs", or "lookup", prefer `agent-retrieval-architecture`.
- If the user says "sync", "recovery", "snapshot", "budget", "quota", "ETag", "background job", or "long-running", prefer `agent-durability-ops`.
- If the user says "remind", "follow up", "weekly review", or "autonomous", prefer `agent-proactive-architecture`.
- If the user says "provider failed", "timeout", "fallback", "degrade", or "trace", prefer `agent-fallback-eval`.
- If the user says "too many questions", "too slow", "too much setup", or "wrong surface", prefer `agent-frictionless-design`.
