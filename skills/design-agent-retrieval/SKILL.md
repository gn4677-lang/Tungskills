...
description: "Use when designing or fixing agent search, retrieval, document lookup, source routing, RAG, deterministic state vs retrieved context, query mode, corpus routing, or fallback full-document reads. Trigger on 搜尋, RAG, 找資料, source attribution, multi-corpus, retrieval eval, or answer quality caused by source selection."
---
...
Use this as a design protocol. Start from the retrieval failure, choose query/source routing boundaries, then return an implementable retrieval contract.
...
- If the core complaint is forgetting or cross-session leakage, route to `design-agent-context`.
