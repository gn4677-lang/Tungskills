...
- Durable workflow recovery belongs in `design-agent-durability` when the main problem is long-running execution survival.
- Retrieval-quality evaluation belongs in `design-agent-retrieval` when the main failure is source selection or grounding.
